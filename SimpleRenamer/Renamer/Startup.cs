namespace Renamer
{
    ...
    public class Startup
    {
        ...
        {
            string[] allfiles = Directory.GetFiles("D:\\Drive\\Music", "*.*", SearchOption.AllDirectories);
            ...
            {
                if (Regex.IsMatch(musicFile.FileName, @"\p{IsCyrillic}"))
                {
                    string renameResult = Rename(musicFile.FileName);
        ...
        private static string Rename(string input)
        {
            ...
            foreach (var letter in input)
            {
                ...
                if (CirilycToLatin.ContainsKey(uppercaseLetter))
                {
                    if (isLowercase)
                    {
                        output.Append(CirilycToLatin[uppercaseLetter].ToLower());
                    }
                    else
                    {
                        output.Append(CirilycToLatin[uppercaseLetter]);
                    }
                }
                else
                {
